---
title: 'Potion Lang'
date: 2019-10-07T02:07:00+01:00
draft: false
---
...
*   A lightweight extremely fast generational copying Cheney GC, based on Basile Starynkevitch's work on Qish. [http://starynkevitch.net/Basile/qishintro.html](http://starynkevitch.net/Basile/qishintro.html)
...
*   It is two languages in one.
    ...
    ```
    app = [window (width=200, height=400) [button "OK", button "Cancel"]]
    ```
...
*   Deeply nested blocks can be closed quickly. I don't like significant whitespace, personally. But I don't like end end end end.
    ...
    ```
     say = (phrase): 10 times (i): 20 times (j): phrase print _say
    ```
    ...
    ```
     say = (): 10 times: 20 times: "Odelay!" print ...
    ```
    
    If you prefer, you can give it some space. Or you can use a variable name introduced by the block,
    
    ```
     say = (phrase): 10 times (i): 20 times (j): phrase print _ phrase say = (phrase): 10 times (i): 20 times (j): phrase print _ i . 
    ```
...
*   Assign is match + bind really (~ planned ~).
    ...
    ```
     (1, x) = (1, 2) => (x=2) (1, x) = (2, 3) => false 1 = 2 => false
    ```
    ...
    ```
     (_, x, 2) = (0, 1, 2) and say x #=> 1 [_, [x, 1]] = [0, [1, 2]] and say x #=> 1 [_, x] = [0, [1, 2]] and say x #=> [1, 2] [_ | x] = [0, 1, 2] and say x #=> [1, 2] fun = (a, b): [0, [a, b]]. [_ | [x, 1]] = fun(1, 2) and say x #=> 1
    ```
...
Of course, Mauricio Fernandez for his inspiring programming journal housed at [eigenclass.org/R2/](http://web.archive.org/web/20110814062722/http://eigenclass.org/R2/) and for works derived throughout the course of it -- extprot most of all. Many of my thoughts about language internals (object repr, GC, etc.) are informed by him.
...
from Hacker News https://ift.tt/1iPWn8F