---
title: 'Fix Two Linux Kernel Bugs While Testing TiDB Operator in K8s'
...
![Kernel path information 1](https://pingcap.com/images/svgs/loader-spinner.svg)

![Kernel path information 2](https://pingcap.com/images/svgs/loader-spinner.svg)
...
**Keyword:** kernel:unregister\_netdevice: waiting for eth0 to become free. Usage count = 1

**Related issues in the community:**

### Issue description

After the K8s platform ran for a time, the following message was displayed: “Kernel:unregister\_netdevice: waiting for eth0 to become free. Usage count = 1”. In addition, multiple processes would be in an uninterruptible state. The only workaround for this problem was to restart the server.
...
As we prepared to drill down into the callback function logic registered by each subscriber, we also followed up on the progress of kernel patches and RHEL. We found that [solutions:3659011](https://access.redhat.com/solutions/3659011) of RHEL had an update that mentioned a patch submitted by the upstream link: [route: set the deleted fnhe fnhe\_daddr to 0 in ip\_del\_fnhe to fix a race](https://github.com/torvalds/linux/commit/ee60ad219f5c7c4fb2f047f88037770063ef785f).
...
from Hacker News https://ift.tt/2ZCaaLD