---
title: 'Compreendendo o Ransomware: Detectando Sodin'
date: 2019-12-19T15:59:00+01:00
draft: false
---

  

1.  [General Techniques](https://hatching.io/blog/ransomware-part1)
2.  Detecting Sodin
3.  [Coming soon](https://twitter.com/hatching_io)
...
Active from early 2019, Sodin has rapidly become a dominant force in ransomware activity, quickly filling the gap left by the end of Gandcrab being available as a service. There are many good writeups of the Sodin family available online, and in this blog post, we are not doing a full analysis of the sample. Instead, we will breakdown the main ways in which Sodin is detectable and identifiable within a dynamic sandbox environment, aiming to give examples of some of the techniques covered in [part 1](https://hatching.io/blog/ransomware-part1) of this miniseries.
...
[https://tria.ge/reports/191216-pg5st7zccj/](https://tria.ge/reports/191216-pg5st7zccj/)

06b323e0b626dc4f051596a39f52c46b35f88ea6f85a56de0fd76ec73c7f3851

[https://tria.ge/reports/191216-4zdx1n374x/](https://tria.ge/reports/191216-4zdx1n374x/)

0fa207940ea53e2b54a2b769d8ab033a6b2c5e08c78bf4d7dade79849960b54d

[https://tria.ge/reports/191216-8bfljdyw2s/](https://tria.ge/reports/191216-8bfljdyw2s/)

139a7d6656feebe539b2cb94b0729602f6218f54fb5b7531b58cfe040f180548

[https://tria.ge/reports/191216-4rcmytrrka/](https://tria.ge/reports/191216-4rcmytrrka/)
...
We took a look at ransom notes in [the last blog post](https://hatching.io/blog/ransomware-part1) in this series so we won't go over the details again here, but in summary, they contain instructions for the victim on how to pay the ransom and decrypt their files.

![/static/images/blog/analyst-notes/ransom_note.png](https://hatching.io/static/images/blog/analyst-notes/ransom_note.png)

_Example ransom note created by Sodin infection_
...
Note that you must pass your API key to access Triage endpoints. This can be found on your [account](https://tria.ge/account) page.
...
We have produced a simple Python script which, when passed a Triage analysis ID and your API key, will perform the process outlined above and dump every .txt file found in the onemon.json to the current directory. It will generate a number of `.txt` files. You can find the script [here](https://hatching.io/static/other/triage-ransomnote.py). To use it, you'll need the Triage ID, the task ID of the analysis you want to examine, and your API key for tria.ge. Usage may look as follows:

```
python3 triage-ransomnote.py  191216\-4rcmytrrka task1  
...
![/static/images/blog/analyst-notes/sodin_ransom_portal.png](https://hatching.io/static/images/blog/analyst-notes/sodin_ransom_portal.png)

_Ransom portal landing page_
...
![ransom_details](https://hatching.io/static/images/blog/analyst-notes/sodin_ransom_portal_details.png) ![trial_decryptor](https://hatching.io/static/images/blog/analyst-notes/sodin_ransom_portal_trial_decrypt.png)

_Main elements of Sodin ransom portal_
...
The image is a bitmap created through the [DrawTextW](https://docs.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-drawtextw) function in User32.dll. The text to be written is defined within the malware's JSON config section (discussed later in this post).

![/static/images/blog/analyst-notes/wallpaper_drawtext.png](https://hatching.io/static/images/blog/analyst-notes/wallpaper_drawtext.png)

_Sample passes string into DrawTextW to create background image_
...
![/static/images/blog/analyst-notes//wallpaper_save.png](https://hatching.io/static/images/blog/analyst-notes//wallpaper_save.png)
...
![/static/images/blog/analyst-notes/system_recovery.png](https://hatching.io/static/images/blog/analyst-notes/system_recovery.png)
...
Sets system to ignore errors and boot as normal (NB: this is also the default Windows setting)
...
![objdump_zeacl](https://hatching.io/static/images/blog/analyst-notes/objdump_zeacl.png) ![objdump_grrr](https://hatching.io/static/images/blog/analyst-notes/objdump_grrr.png)
...
Only used if net field is also set, sent to C2 servers. Likely related to campaign identifier etc.
...
Boolean. If set, sample will try to erase contents of folders blacklisted in the wfld field
...
List of folder names. If the wipe field is set to true then the malware will attempt to erase the contents of all folders listed here

prc

List of process names the malware will try to terminate before carrying out file encryption
...
Boolean value. Sets whether sample should send host information to C2 servers listed in dmn key

nbody

Base64-encoded version of the ransom note dropped to the file system after encryption
...
Boolean value. Defines whether or not the malware will use an exploit to try and escalate privileges on the system

img

Base64-encoded version of the text shown in the wallpaper background set by the malware.
...
![/static/images/blog/analyst-notes/unpacker_create_stub.png](https://hatching.io/static/images/blog/analyst-notes/unpacker_create_stub.png)
...
![/static/images/blog/analyst-notes/config_decrypt_start.png](https://hatching.io/static/images/blog/analyst-notes/config_decrypt_start.png)

_Preparing keylength and ciphertext parameters and passing them to the configuration decryption function_
...
The Sodin extractor is not available on Triage yet while testing and improvements to the memory dumping methodology are implemented, but keep an eye on our [Twitter account](https://twitter.com/hatching_io) for updates when that is released.
...
![/static/images/blog/analyst-notes/saves_file_extension.png](https://hatching.io/static/images/blog/analyst-notes/saves_file_extension.png)

_File extension saved to SOFTWARE\\WOW6432Node\\recfg\\rnd\_ext_
...
![/static/images/blog/analyst-notes/recfg_registry_changes.png](https://hatching.io/static/images/blog/analyst-notes/recfg_registry_changes.png)

_Values within the recfg registry key_
...
![/static/images/blog/analyst-notes/registry_stat_key.png](https://hatching.io/static/images/blog/analyst-notes/registry_stat_key.png)

_Tria.ge signature output for registry keys_
