---
title: 'Raspberry Pi E Ink Task
Organizer #piday #raspberrypi @Raspberry_Pi #Eink #3DPrinting @Element14'
date: 2019-12-07T01:41:00+01:00
draft: false
---

From [Element14](https://www.youtube.com/channel/UChturLXwYxwTOf_5krs0qvA) on [YouTube](https://youtu.be/EhCJYlPNy3E):

> Design and build of a Rasbperry PI based Calendar, ToDo and Weather display on an e-ink screen. The case is made of all 3d printed parts and is able to be set on a desktop, or hung upside down from a shelf. All code written in python and pulls data from Open Weather Map for weather, Google Calendar for the calendar items and Todoist for ToDo items: [http://bit.ly/2QRhKQg](http://bit.ly/2QRhKQg)

[See more!](https://youtu.be/EhCJYlPNy3E)

* * *

[![3055 06](https://cdn-blog.adafruit.com/uploads/2017/01/3055-06.jpg "3055-06.jpg")](https://www.adafruit.com/raspberrypi)Each Friday is [PiDay](http://www.adafruit.com/blog/?main_page=blog&s=%23piday) here at Adafruit! Be sure to check out our [posts](http://www.adafruit.com/blog/category/raspberry-pi/), [tutorials](http://learn.adafruit.com/category/raspberry-pi) and new [Raspberry Pi related products](http://www.adafruit.com/raspberrypi). Adafruit has the largest and best selection of Raspberry Pi accessories and all the code & tutorials to get you up and running in no time!