---
title: 'Ffuf'
date: 2019-12-19T11:46:00+01:00
draft: false
---

**Kang Asu**

[![](https://1.bp.blogspot.com/-6uGHTdIePDQ/XfA2L2mrnZI/AAAAAAAARD4/5Y9ri5OL62QR7FVPTF4d_X76Qhulu3cSACNcBGAsYHQ/s640/Web%2BFuzzer%2B1.png)](https://asciinema.org/a/211350)

**Ffuf - Fast Web Fuzzer Written In Go**

A fast web fuzzer written in Go.  
Heavily inspired by the great projects [gobuster](https://github.com/OJ/gobuster "gobuster") and [wfuzz](https://github.com/xmendez/wfuzz "wfuzz").  
...
**Virtual [host discovery](https://www.kitploit.com/search/label/Host%20Discovery "host discovery") (without DNS records)**  

[![](https://1.bp.blogspot.com/-1OXxtA6zA7Y/XfA2iqKOfEI/AAAAAAAAREA/V1KPQ7gC6BoSIrbCuzkMdTqG1_bkNChFgCNcBGAsYHQ/s640/Web%2BFuzzer%2B2.png)](https://asciinema.org/a/211360)

  
Assuming that the default virtualhost response size is 4242 bytes, we can filter out all the responses of that size (`-fs 4242`)while fuzzing the Host - header:

```
ffuf -w /path/to/vhost/wordlist -u [https://target](https://target/) -H "Host: FUZZ" -fs 4242
```
...
```
ffuf -w /path/to/paramnames.txt -u [https://target/script.php?FUZZ=test_value](https://target/script.php?FUZZ=test_value) -fs 4242
```
...
For this example, we'll fuzz JSON data that's sent over POST. [Radamsa](https://gitlab.com/akihe/radamsa "Radamsa") is used as the mutator.  
...
```
ffuf --input-cmd 'radamsa --seed $FFUF_NUM example1.txt example2.txt' -H "Content-Type: application/json" -X POST -u [https://ffuf.io.fi/](https://ffuf.io.fi/) -mc all -fc 400
```

It of course isn't very efficient to call the mutator for each payload, so we can also pre-generate the payloads, still using [Radamsa](https://gitlab.com/akihe/radamsa "Radamsa") as an example:
...
ffuf --input-cmd 'cat $FFUF_NUM.txt' -H "Content-Type: application/json" -X POST -u [https://ffuf.io.fi/](https://ffuf.io.fi/) -mc all -fc 400
```
...
eg. `ffuf -u [https://example.org/FUZZ](https://example.org/FUZZ) -w /path/to/wordlist`  
  
**Installation**

*   [Download](https://github.com/ffuf/ffuf/releases/latest "Download") a prebuilt binary from [releases page](https://github.com/ffuf/ffuf/releases/latest "releases page"), unpack and run! or
*   If you have go compiler installed: `go get [github.com/ffuf/ffuf](http://github.com/ffuf/ffuf)`
...
*    v0.12  
    *   New
        ...
        *   HTML output format got [DataTables](https://datatables.net/ "DataTables") support allowing realtime searches, sorting by column etc.
...
*    v0.11  
    *    New  
        ...
        *   Ability to use multiple [wordlists](https://www.kitploit.com/search/label/Wordlists "wordlists") / keywords by defining multiple -w [command line](https://www.kitploit.com/search/label/Command%20Line "command line") flags. The if no keyword is defined, the default is FUZZ to keep backwards compatibility. Example: `-w "wordlists/custom.txt:CUSTOM" -H "RandomHeader: CUSTOM"`.
    *    Changed  
...
*    v0.10  
    *    New  
        ...
        *   New CLI flags: --input-cmd, and --input-num to handle input generation using external commands. Mutators for example. Environment variable FFUF\_NUM will be updated on every call of the command.
...
**[Download Ffuf](http://locinealy.com/4XQp "Download Ffuf")**
