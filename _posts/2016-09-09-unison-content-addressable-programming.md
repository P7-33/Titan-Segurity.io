...
A tour of Unison[#](https://www.unisonweb.org/docs/tour/#a-tour-of-unison)
==========================================================================

Introduction[#](https://www.unisonweb.org/docs/tour/#introduction)
------------------------------------------------------------------
...
If you want to follow along with this document (highly recommended), this guide assumes you've already gone through the steps in [the quickstart guide](https://www.unisonweb.org/docs/quickstart).

The source for this document is [on GitHub](https://github.com/unisonweb/docsite/blob/gh-pages/_includes/unisontour.markdown). Feedback and improvements are most welcome!

The most important goal[#](https://www.unisonweb.org/docs/tour/#the-most-important-goal)
----------------------------------------------------------------------------------------
...
We **do** need something else to make it nice to work with content-addressed code. In Unison we call this something else the _Unison Codebase Manager_.

👋 to the Unison codebase manager[#](https://www.unisonweb.org/docs/tour/#-to-the-unison-codebase-manager)
----------------------------------------------------------------------------------------------------------
...
If you haven't already worked through the [quickstart guide](https://www.unisonweb.org/docs/quickstart), let's download the Unison base library to the `.base` namespace:
...
```
.> find : [a] -> [a] 1. base.Heap.sort : [a] -> [a] 2. base.List.distinct : [a] -> [a] 3. base.List.reverse : [a] -> [a] 4. base.Heap.sortDescending : [a] -> [a] .> view 3 base.List.reverse : [a] -> [a] base.List.reverse as = use base.List +: base.List.foldl (acc a -> a +: acc) [] as
```
...
*   `base.List.reverse : [a] -> [a]` is the syntax for giving a [type signature](https://www.unisonweb.org/docs/language-reference/term-declarations/#type-signatures) to a definition. We pronounce the `:` symbol as "has type", as in "reverse has the type `[a] -> [a]`".
...
*   Any lowercase variable in a type signature is assumed to be [universally quantified](https://www.unisonweb.org/docs/language-reference/types/#polymorphic-types), so `[a] -> [a]` really means and could be written `forall a . [a] -> [a]`, which is the type of functions that take a list whose elements are some (but any) type, and return a list of elements of that same type.
*   `base.List.reverse` takes one parameter, called `as`. The stuff after the `=` is called the _body_ of the function, and here it's a [block](https://www.unisonweb.org/docs/language-reference/blocks), which is demarcated by whitespace.
...
*   The declaration `use base.List +:` lets us reference the function `base.List.+:` using just `+:`. (This function prepends an element to the front of a list.) [Use clauses](https://www.unisonweb.org/docs/language-reference/use) like this can be placed in any Unison block; they don't need to go at the top of your file.

> Try doing `view base.List.foldl` if you're curious to see how it's defined.

### Names are stored separately from definitions so renaming is fast and 100% accurate[#](https://www.unisonweb.org/docs/tour/#names-are-stored-separately-from-definitions-so-renaming-is-fast-and-100-accurate)
...
```
.> move.term base.List.foldl base.List.foldLeft Done. .> view base.List.reverse base.List.reverse : [a] -> [a] base.List.reverse as = use base.List +: base.List.foldLeft (acc a -> a +: acc) [] as
```
...
> 
> 🤓 If you're curious to learn about the guts of the Unison codebase format, you can check out the [v1 codebase format specification](https://github.com/unisonweb/unison/blob/master/docs/repoformats/v1-DRAFT.markdown).

Use `undo` to back up a step. (We don't have a `redo` yet, though).

```
.> undo ⏪ Here's the changes I undid: > Moves: Original name New name base.List.foldl base.List.foldLeft .>
```

Great! OK, go drink some water, and then let's learn more about Unison's interactive way of writing and editing code.

Unison's interactive scratch files[#](https://www.unisonweb.org/docs/tour/#unisons-interactive-scratch-files)
-------------------------------------------------------------------------------------------------------------
...
Keep your `ucm` terminal running and open up a file, `scratch.u` (or `foo.u`, or whatever you like) in your preferred text editor (if you want syntax highlighting for Unison files, [follow this link](https://www.unisonweb.org/docs/editor-setup) for instructions on setting up your editor).

Now put the following in your scratch file:

```
use .base square : Nat -> Nat square x = x * x
```
...
```
-- A comment, ignored by Unison > base.List.reverse [1,2,3,4] > 4 + 6 > 5.0 / 2.0 > not true
```

```
✅ ~/unisoncode/scratch.u changed. Now evaluating any watch expressions (lines starting with `>`)... Ctrl+C cancels. 6 | > base.List.reverse [1,2,3,4] ⧩ [4, 3, 2, 1] 7 | > 4 + 6 ⧩ 10 8 | > 5.0 / 2.0 ⧩ 2.5 9 | > not true ⧩ false
```

Testing your code[#](https://www.unisonweb.org/docs/tour/#testing-your-code)
----------------------------------------------------------------------------

Let's add a test for our `square` function:

```
use .base square : Nat -> Nat square x = x * x use test.v1 test> tests.square.ex1 = run (expect (square 4 == 16))
```

Save the file, and Unison comes back with:

```
7 | test> tests.square.ex1 = run (expect (square 4 == 16)) ✅ Passed : Passed 1 tests.
```
...
### A property-based test[#](https://www.unisonweb.org/docs/tour/#a-property-based-test)
...
```
use .base square : Nat -> Nat square x = x * x use test.v1 test> tests.square.ex1 = run (expect (square 4 == 16)) test> tests.square.prop1 = go _ = a = !nat b = !nat expect (square a * square b == square (a * b)) runs 100 go
```

```
8 | go _ = a = !nat ✅ Passed : Passed 100 tests. (cached)
```

This will test our function with a bunch of different inputs.

#### Syntax notes[#](https://www.unisonweb.org/docs/tour/#syntax-notes)
...
*   You can call a function parameter `_` if you just plan to ignore it. Here, `go` ignores its argument; its purpose is just to make `go` [lazily evaluated](https://www.unisonweb.org/docs/language-reference/expressions/#delayed-computations) so it can be run multiple times by the `runs` function.
*   `!expr` means the same thing as `expr ()`, we say that `!expr` _forces_ the [delayed computation](https://www.unisonweb.org/docs/language-reference/expressions/#delayed-computations) `expr`.
...
Adding code to the codebase[#](https://www.unisonweb.org/docs/tour/#adding-code-to-the-codebase)
------------------------------------------------------------------------------------------------
...
```
.> add ⍟ I've added these definitions: tests.square.ex1 : [base.Test.Result] tests.square.prop1 : [base.Test.Result] square : base.Nat -> base.Nat
```
...
```
.> test Cached test results (`help testcache` to learn more) ◉ tests.square.ex1 : Passed 1 tests. ◉ tests.square.prop1 : Passed 100 tests. ✅ 2 test(s) passing Tip: Use view tests.square.ex1 to view the source of a test.
```
...
Unison namespaces and use clauses[#](https://www.unisonweb.org/docs/tour/#unison-namespaces-and-use-clauses)
------------------------------------------------------------------------------------------------------------
...
We often think of these names as forming a tree, much like a directory of files, and names are like file paths in this tree. [Absolute names](https://www.unisonweb.org/docs/language-reference/identifiers/#absolutely-qualified-identifiers) (like `.base.Int`) start with a `.` and are paths from the root of this tree and _relative_ names (like `math.sqrt`) are paths starting from the current namespace, which you can set using the `namespace` (or equivalently `cd`) command:

```
.> namespace mylibrary ☝️ The namespace .mylibrary is empty. .mylibrary>
```
...
```
.mylibrary> move.term .square square Done. .mylibrary> find 1. square : .base.Nat -> .base.Nat .mylibrary> move.namespace .tests tests Done.
```
...
```
.mylibrary> find 1. tests.square.ex1 : [.base.Test.Result] 2. tests.square.prop1 : [.base.Test.Result] 3. square : .base.Nat -> .base.Nat
```

Also notice that we don't need to rerun our tests after this reshuffling. The tests are still cached:

```
.mylibrary> test Cached test results (`help testcache` to learn more) ◉ tests.square.ex1 : Passed 1 tests. ◉ tests.square.prop1 : Passed 100 tests. ✅ 2 test(s) passing Tip: Use view tests.square.ex1 to view the source of a test.
```
...
Modifying existing definitions[#](https://www.unisonweb.org/docs/tour/#modifying-existing-definitions)
------------------------------------------------------------------------------------------------------
...
### Using the `edit` command[#](https://www.unisonweb.org/docs/tour/#using-the-edit-command)

Try doing `edit square` from your prompt (note you can use tab completion):

```
.mylibrary> edit square ☝️ I added these definitions to the top of ~/unisoncode/scratch.u square : .base.Nat -> .base.Nat square x = use .base.Nat * x * x You can edit them there, then do `update` to replace the definitions currently in this branch.
```
...
```
use .base square : Nat -> Nat square x = sum (map (x -> x * 2 + 1) (range 0 x)) sum : [Nat] -> Nat sum = foldl (+) 0
```

```
✅ I found and typechecked these definitions in ~/unisoncode/scratch.u. If you do an `add` or `update` , here's how your codebase would change: ⍟ These new definitions will replace existing ones of the same name and are ok to `update`: square : .base.Nat -> .base.Nat Now evaluating any watch expressions (lines starting with `>`)... Ctrl+C cancels.
```

### Adding an updated definition to the codebase[#](https://www.unisonweb.org/docs/tour/#adding-an-updated-definition-to-the-codebase)

Notice the message says that `square` is "ok to `update`". Let's try that:

```
.mylibrary> update ⍟ I've added these definitions: sum : [.base.Nat] -> .base.Nat ⍟ I've updated to these definitions: square : .base.Nat -> .base.Nat
```

### Only affected tests are rerun on `update`[#](https://www.unisonweb.org/docs/tour/#only-affected-tests-are-rerun-on-update)

If we rerun the tests, the tests won't be cached this time, since one of their dependencies has actually changed:

```
.mylibrary> test New test results: ◉ tests.square.prop1 : Passed 100 tests. ◉ tests.square.ex1 : Passed 1 tests. ✅ 2 test(s) passing Tip: Use view tests.square.prop1 to view the source of a test.
```
...
Publishing code and installing Unison libraries[#](https://www.unisonweb.org/docs/tour/#publishing-code-and-installing-unison-libraries)
----------------------------------------------------------------------------------------------------------------------------------------
...
```
.mylibrary> cd . .> copy.namespace mylibrary mylibrary.releases.v1 Done. .> cd mylibrary.releases.v1 .mylibrary.releases.v1> find 1. tests.square.ex1 : [Result] 2. tests.square.prop1 : [Result] 3. square : Nat -> Nat
```
...
After you've created this empty repo, you can then push to it, using `push` (to push the current namespace) or `push .mystuff` (to push the `.mystuff` namespace):

```
.mylibrary.releases.v1> cd .mylibrary .mylibrary> push git@github.com:/myunisonrepo
```

You'll see some git logging output. Your code is now live on the internet!

Installing libraries written by others[#](https://www.unisonweb.org/docs/tour/#installing-libraries-written-by-others)
----------------------------------------------------------------------------------------------------------------------
...
```
.> pull git@github.com:/myunisonrepo.git .myfirstlibrary
```

The namespace you created is now available under `.myfirstlibrary`. Try `cd .myfirstlibrary` after the pull to look around.

What next?[#](https://www.unisonweb.org/docs/tour/#what-next)
-------------------------------------------------------------

*   [The core language reference](https://www.unisonweb.org/docs/language-reference) describes Unison's core language and current syntax in more detail.
*   TODO: writing a more interesting library

  
  
from Hacker News https://ift.tt/2RuHNgb