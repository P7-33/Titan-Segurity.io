...
Plugin v3.0.8'
date: 2019-09-26T06:49:00+01:00
draft: false
---

**Kang Asu**

[![](https://1.bp.blogspot.com/-yqVbvEdGolc/XX3R39NA-AI/AAAAAAAABsA/KAcEDbhyHu8hzsr8avONS94eW1GnQS6dgCLcBGAsYHQ/s320/RDP-MITM-How-it-works.png)](https://1.bp.blogspot.com/-yqVbvEdGolc/XX3R39NA-AI/AAAAAAAABsA/KAcEDbhyHu8hzsr8avONS94eW1GnQS6dgCLcBGAsYHQ/s1600/RDP-MITM-How-it-works.png)

**Pyrdp - RDP Man-In-The-Middle And Library For Python3 With The Ability To Watch Connections Live Or After The Fact**

PyRDP is a Python 3 [Remote Desktop Protocol](https://www.kitploit.com/search/label/Remote%20Desktop%20Protocol "Remote Desktop Protocol") (RDP) [Man-in-the-Middle](https://www.kitploit.com/search/label/Man-in-the-Middle "Man-in-the-Middle") (MITM) and library.  
It features a few tools:

*   RDP Man-in-the-Middle
    *   Logs [credentials](https://www.kitploit.com/search/label/Credentials "credentials") used when connecting
    ...
    *   Run console commands or [PowerShell](https://www.kitploit.com/search/label/PowerShell "PowerShell") payloads automatically on new connections
*   RDP Player:
...
PyRDP was [first introduced in a blogpost](https://www.gosecure.net/blog/2018/12/19/rdp-man-in-the-middle-smile-youre-on-camera "first introduced in a blogpost") in which we [demonstrated that we can catch a real threat actor in action](https://www.youtube.com/watch?v=eB7RC9FmL6Q "demonstrated that we can catch a real threat actor in action"). In May 2019 a [presentation by its authors](https://docs.google.com/presentation/d/1avcn8Sh2b3IE7AA0G9l7Cj5F1pxqizUm98IbXUo2cvY/edit#slide=id.g404b70030f_0_581 "presentation by its authors") was given at NorthSec and two demos were performed. [The first one covered](https://youtu.be/5JztJzi-m48 "The first one covered") credential logging, clipboard stealing, client-side file browsing and a session take-over. [The second one covered](https://youtu.be/bU67tj1RkMA "The second one covered") the execution of cmd or powershell payloads when a client successfully authenticates. In August 2019, PyRDP was demo'ed at BlackHat Arsenal ([slides](https://docs.google.com/presentation/d/17P_l2n-hgCehQ5eTWilru4IXXHnGIRTj4ftoW4BiX5A/edit?usp=sharing "slides")).  
...
This tool has been tested to work on Python 3.6 on Linux (Ubuntu 18.04) and Windows (See section [Installing on Windows](https://github.com/gosecure/pyrdp#installing-on-windows "Installing on Windows")). It has not been tested on OSX.  
  
**Installing**  
We recommend installing PyRDP in a [virtual environment](https://packaging.python.org/guides/installing-using-pip-and-virtual-environments/ "virtual environment") to avoid dependency issues.  
...
_DO NOT_ use the root PyRDP directory for the virtual environment folder (`python3 -m venv .`). You will make a mess, and using a directory name like `venv` is more standard anyway.  
...
1.  Any C compiler
2.  [OpenSSL](https://wiki.openssl.org/index.php/Binaries "OpenSSL"). Make sure it is reachable from your `$PATH`.
...
*   [Using the PyRDP MITM](https://github.com/gosecure/pyrdp#using-the-pyrdp-man-in-the-middle "Using the PyRDP MITM")
*   [Using the PyRDP Player](https://github.com/gosecure/pyrdp#using-the-pyrdp-player "Using the PyRDP Player")
*   [Using the PyRDP Certificate Cloner](https://github.com/gosecure/pyrdp#using-the-pyrdp-certificate-cloner "Using the PyRDP Certificate Cloner")
...
If you are running the MITM on a server and still want to see live RDP connections, you should use [SSH remote port forwarding](https://www.booleanworld.com/guide-ssh-port-forwarding-tunnelling/ "SSH remote port forwarding") to forward a port on your server to the player's port on your machine. Once this is done, you pass `127.0.0.1` and the forwarded port as arguments to the MITM. For example, if port 4000 on the server is forwarded to the player's port on your machine, this would be the command to use:
...
4.  Run the payload as a console command and exit the console. If a PowerShell payload is configured, it is run with `powershell -enc` .
...
This could be made more accurate by leveraging some messages exchanged during RDPDR initialization. See [this issue](https://github.com/GoSecure/pyrdp/issues/98 "this issue") if you're interested in making this work better.  
...
If you're interested in experimenting with RDP and making your own tools, head over to our [documentation section](https://github.com/GoSecure/pyrdp/blob/master/docs/README.md "documentation section") for more information.  
  
**Using PyRDP with Bettercap**  
We developped our own Bettercap module, `rdp.proxy`, to man-in-the-middle all RDP connections on a given LAN. Check out [this document](https://github.com/GoSecure/pyrdp/blob/master/docs/bettercap-rdp-mitm.md "this document") for more information.  
  
**PyRDP Presentations**  
  
**Contributing to PyRDP**  
See our [contribution guidelines](https://github.com/GoSecure/pyrdp/blob/master/CONTRIBUTING.md "contribution guidelines").  
  
**Acknowledgements**  
PyRDP uses code from the following open-source software:

*   [RC4-Python](https://github.com/bozhu/RC4-Python "RC4-Python") for the RC4 implementation.
*   [rdesktop](https://github.com/rdesktop/rdesktop "rdesktop") for bitmap decompression.
*   [rdpy](https://github.com/citronneur/rdpy "rdpy") for RC4 keys, the bitmap decompression bindings and the base GUI code for the PyRDP player.
*   [FreeRDP](https://github.com/FreeRDP/FreeRDP "FreeRDP") for the scan code enumeration.

  

**[Download Pyrdp](http://eunsetee.com/9kHV "Download Pyrdp")**
